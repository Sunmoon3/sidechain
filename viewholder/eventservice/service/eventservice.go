...
import (
	...
	"git.labpc.bluarry.top/bluarry/viewholder/eventservice/helpers"
	"git.labpc.bluarry.top/bluarry/viewholder/eventservice/service/dispatcher"
...
func New(dispatcher Dispatcher,opts ...common.Opt) *Service {
...
func (s *Service) StopAndTransfer() (common.EventSnapshot, error) {
	eventch, err := s.dispatcher.EventCh()
	if err != nil {
		logger.Warnf("Error stopping event service: %s", err)
		return nil, err
	...
	snapshotch := make(chan common.EventSnapshot, 1)
	errch := make(chan error, 1)
...
func (s *Service) Transfer() (common.EventSnapshot, error) {
	eventch, err := s.dispatcher.EventCh()
	if err != nil {
...
func (s *Service) RegisterBlockEvent(filter ...common.BlockFilter) (common.Registration, <-chan *common.BlockEvent, error) {
	eventch := make(chan *common.BlockEvent, s.eventConsumerBufferSize)
	regch := make(chan common.Registration)
	errch := make(chan error)

	blockFilter := helpers.AcceptAny
	if len(filter) > 1 {
...
func (s *Service) RegisterFilteredBlockEvent() (common.Registration, <-chan *common.FilteredBlockEvent, error) {
	eventch := make(chan *common.FilteredBlockEvent, s.eventConsumerBufferSize)
	regch := make(chan common.Registration)
	errch := make(chan error)
...
func (s *Service) RegisterChaincodeEvent(ccID, eventFilter string) (common.Registration, <-chan *common.CCEvent, error) {
	if ccID == "" {
	...
	eventch := make(chan *common.CCEvent, s.eventConsumerBufferSize)
	regch := make(chan common.Registration)
	errch := make(chan error)
...
func (s *Service) RegisterTxStatusEvent(txID string) (common.Registration, <-chan *common.TxStatusEvent, error) {
	if txID == "" {
		return nil, nil, errors.New("txID must be provided")
	}

	eventch := make(chan *common.TxStatusEvent, s.eventConsumerBufferSize)
	regch := make(chan common.Registration)
	errch := make(chan error)
...
func (s *Service) Unregister(reg common.Registration) {
