...
import (
	...
	"git.labpc.bluarry.top/bluarry/viewholder/eventservice/helpers/txflags"
...
func New(opts ...common.Opt)*Dispatcher{
...
func (ed *Dispatcher) RegisterHandlers() {
	...
	ed.RegisterHandler(&common.BlockEvent{}, ed.handleBlockEvent)
	ed.RegisterHandler(&common.FilteredBlockEvent{}, ed.handleFilteredBlockEvent)
}

func (ed *Dispatcher) handleBlockEvent(e Event) {
	evt := e.(*common.BlockEvent)
	ed.HandleBlock(evt.Block, evt.SourceURL)
}
func (ed *Dispatcher) handleFilteredBlockEvent(e Event) {
	evt := e.(*common.FilteredBlockEvent)
...
func (ed *Dispatcher) newSnapshot() common.EventSnapshot {
